...
class PollTargetPhaseTest {
    ...
    fun `phase offset is deterministic for the same target`() {
        val a = PollTargetPhase.phaseOffsetMs(TargetType.CAMPGROUND, Provider.RECREATION_GOV, "233359", 120000)
        val b = PollTargetPhase.phaseOffsetMs(TargetType.CAMPGROUND, Provider.RECREATION_GOV, "233359", 120000)
        assertEquals(a, b)
    }

    @Test
    fun `phase offset differs across different targets in general`() {
        val offsets = (1..50).map { PollTargetPhase.phaseOffsetMs(TargetType.CAMPGROUND, Provider.RECREATION_GOV, "campsite-$it", 120000) }
    ...
    fun `phase offset is always within the interval bounds`() {
        repeat(200) { i ->
            val offset = PollTargetPhase.phaseOffsetMs(TargetType.PERMIT, Provider.RECREATION_GOV, "permit-$i", 120000)
    ...
    fun `target type is part of the hash input, not just target id`() {
        ...
        val differsForAtLeastOne = (1..50).any { i ->
            PollTargetPhase.phaseOffsetMs(TargetType.CAMPGROUND, Provider.RECREATION_GOV, "$i", 120000) !=
                PollTargetPhase.phaseOffsetMs(TargetType.PERMIT, Provider.RECREATION_GOV, "$i", 120000)
        }
    ...
    fun `rejects non-positive interval`() {
        org.junit.jupiter.api.Assertions.assertThrows(IllegalArgumentException::class.java) {
            PollTargetPhase.phaseOffsetMs(TargetType.CAMPGROUND, Provider.RECREATION_GOV, "1", 0)
        }
    }
}
