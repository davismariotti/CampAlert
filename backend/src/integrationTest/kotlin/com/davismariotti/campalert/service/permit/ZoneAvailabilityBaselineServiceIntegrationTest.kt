...
class ZoneAvailabilityBaselineServiceIntegrationTest : IntegrationTestBase() {
    ...
    private fun gates(total: Int, remaining: Int) = mapOf("default" to AvailabilityQuotaGate(total, remaining))

    @Test
    fun `first tick for a division is never suspicious and persists a baseline in Redis`() {
        val result = service.looksSuspicious("233261", "343", month, mapOf(date to gates(total = 25, remaining = 3)))
    ...
    fun `a date that flips from partially-booked to fully-open across real ticks is flagged suspicious`() {
        service.looksSuspicious("233261", "343", month, mapOf(date to gates(total = 25, remaining = 3)))

        val result = service.looksSuspicious("233261", "343", month, mapOf(date to gates(total = 25, remaining = 25)))
    ...
    fun `a date that stays fully-open across real ticks is not flagged`() {
        service.looksSuspicious("233261", "343", month, mapOf(date to gates(total = 25, remaining = 25)))
    ...
    fun `baselines for different divisions on the same permit and month are stored independently`() {
        ...
        val result = service.looksSuspicious("233261", "290", month, mapOf(date to gates(total = 25, remaining = 25)))
